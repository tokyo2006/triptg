...
function initGrid(){
	...
	var objectReader = new Ext.data.JsonReader({
        ...
        fields: [
			...
			{name: 'isZone', mapping:'isZone'}
        ]
    });
	
	objectStore = new Ext.data.GroupingStore({
            reader: objectReader,
			proxy: new Ext.data.HttpProxy({url: 'getAllCity.shtml',method:'POST'}),
			remoteSort: true,
	...
	objectGrid = new Ext.grid.GridPanel({
		...
		columns: [
		     rn, //行号列 
		     cbsm, 
            {id:'cityId',header: '标识号', hidden:true, hideable:false, sortable: true, 
			dataIndex: 'cityId'},
			{id:'areaId',header: 'area标识号', hidden:true, hideable:false, sortable: true, 
			dataIndex: 'areaId'},
			...
			dataIndex: 'isTop'},
			{header: '是否城市', hideable:true, sortable: true, renderer:function(v,p,record){
				if(parseInt(v)===1){
					return '是';
				}
				else{
...
function updateItem(idName,start){
	...
	if (!selectRecord) {
		Ext.MessageBox.alert('修改操作', '请选择要修改的城市！');
	}
	else {
		if (cbsm.getCount() == 1) {
			updateTab(selectRecord.get('cityId'), selectRecord.get('name'), 'getSingleCity.shtml?cityId=' + selectRecord.get('cityId'));
		}
		else {
			Ext.MessageBox.alert('提示', '请选择一条城市信息！');
		}
	}
}
function viewItem(start){
	selectRecord = objectGrid.getSelectionModel().getSelected();
	if (!selectRecord) {
		Ext.MessageBox.alert('查看操作', '请选择要查看的城市！');
	}
	else {
		if (cbsm.getCount() == 1) {
