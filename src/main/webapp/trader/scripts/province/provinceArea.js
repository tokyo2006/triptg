 ...
 */
function initGrid(){
	var idName = new Array('name','headName','orderBy','ip','isChina');
	...
	var objectReader = new Ext.data.JsonReader({
        ...
        fields: [
			...
			{name: 'isChina', mapping:'area.isChina'},
	...
	objectStore = new Ext.data.GroupingStore({
            reader: objectReader,
			proxy: new Ext.data.HttpProxy({url: 'getAllProvince.shtml',method:'POST'}),
			remoteSort: true,
	...
	objectGrid = new Ext.grid.GridPanel({
		...
		columns: [
		     rn, //行号列 
		     cbsm, 
            {id:'provinceId',header: '标识号', hidden:true, hideable:false, sortable: true, 
			dataIndex: 'provinceId'},
			{id:'areaId',header: '地域标识号', hidden:true, hideable:false, sortable: true, 
			dataIndex: 'areaId'},
			{id:'isChina',header: '是否国内', hidden:true, hideable:true, sortable: true, 
			dataIndex: 'isChina'},
			...
			dataIndex: 'synopsis'},
			{header: '概况', hideable:true, sortable: true,
			dataIndex: 'gloze'},
...
function updateItem(idName,start){
	...
	if (!selectRecord) {
		Ext.MessageBox.alert('修改操作', '请选择要修改的省份！');
	}
	else {
		if (cbsm.getCount() == 1) {			
			updateTab(selectRecord.get('provinceId'),selectRecord.get('name'),'getSingleProvince.shtml?provinceId='+selectRecord.get('provinceId'));
		}
		else {
			Ext.MessageBox.alert('提示', '请选择一条省份信息！');
		}
	}
}
...
function viewItem(start){
	selectRecord = objectGrid.getSelectionModel().getSelected();
	if (!selectRecord) {
		Ext.MessageBox.alert('查看操作', '请选择要查看的省份！');
	}
	else {
		if (cbsm.getCount() == 1) {
			if(selectRecord.get('isChina')===1){
				window.open('../../destination/proviceDetail/' + selectRecord.get('areaId')+'.html');
			}
			else{
				window.open('../../destination/cityDetail/' + selectRecord.get('areaId')+'.html');
			}
		}
		else {
			Ext.MessageBox.alert('提示', '请选择一条大洲信息！');
		}
	}
}