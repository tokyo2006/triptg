...
//	var sortButton;	
// 	 if(parseInt(list.herfRegion)===1){
//	 	sortButton={
//				text: '类别刷新',
//				tooltip: '刷新类别，建议开班修改后仅刷新一次不要重复刷新',
//				iconCls: 'option',
//				handler: function(){
//				sortRefresh();
//				}
...
//			'-', sortButton
			...
			'->',
			searchCheckbox,searchText,startDateText,endDateText,searchSelect,
			{
				text:'简单搜索',
				handler:function(){
					simpleSearch(searchCheckbox,searchText,startDateText,endDateText,searchSelect,objectStore);
				}
			},'-',{
				text:'高级搜索',
				handler:function(){
					complicatedSearch(searchText,startDateText,endDateText,objectStore);
				}
			}
		],
...
function copyItem(){
	selectRecord = objectGrid.getSelectionModel().getSelected();
	if (!selectRecord) {
		Ext.MessageBox.alert('开班复制操作', '未选择，请选择要复制的开班！');
	}
	else {
...
function copyAjax(jsonData){
	Ext.Ajax.request({
		...
		success: function(result, request){
			Ext.MessageBox.alert('开班复制操作', '复制开班成功！');
			objectStore.load({
					params: {
						start: start,
						limit: pageSize
		...
		failure: function(result, request){
			Ext.MessageBox.alert('开班复制操作', '复制开班失败！');
		}
	});
}*/
...
function complicatedSearch(searchText,startDateText,endDateText,objectStore){
	var beginDate,endDate;
	if(startDateText.getValue()===''){
		beginDate='';
	}
	...
	comSearchItem(beginDate,endDate,simSearch,jobberName,teamName);
}
...
function comSearchItem(beginDate,endDate,simSearch,teamName){
	var comFPanel = new Ext.FormPanel({
		...
		buttons: [{
			text: '确定',
			handler: function(){
				searchOk(comFPanel,beginDate,endDate,simSearch,jobberName,teamName);
			}
		}, {
			text: '取消',
			handler: function(){
...
function searchOk(comFPanel,beginDate,endDate,simSearch,jobberName,teamName){
	var comWin = Ext.getCmp('comW');
	var isJSpe,isJCom,isJNew,isSpe,isCom,isNew;
	if(comFPanel.findById('isJSpe').getValue()){
		isJSpe='1';
