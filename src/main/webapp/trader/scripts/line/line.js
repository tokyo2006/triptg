...
function initData(result){
  ...
  initMenu(regionJosn.menuRegion);
		...
		Ext.MessageBox.alert('加载失败', returnJosn.msg);
	}
}
...
function initView(){
	...
	centerbody =new Ext.TabPanel({
			...
			items:[{
				...
				autoWidth:true,
				frame:true,
				autoLoad: {url: "tabFrameLine.jsp?url="+'totalLine.jsp', callback: this.initSearch, scope: this,  text: "Loading..."}		
			}]
		});
...
{
	updateTab(item.id,item.text,'lineCenter.jsp?params='+item.id+'@@@'+item.stateId);
}
function addTab(id,tabTitle,targetUrl){
	centerbody.add({
		...
		frame:true,
		tabTip:tabTitle,
		autoLoad: {url: "tabFrameLine.jsp?url="+targetUrl, callback: this.initSearch, scope: this,  text: "Loading..."},
		closable:true
		}).show();
}
// 更新tab内容，如不存在就新建一个
