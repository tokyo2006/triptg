...
function initGrid(){
	...
	var objectReader = new Ext.data.JsonReader({
		...
		idProperty:'continentId',
        fields: [
            {name: 'continentId', mapping:'continentId'},
	...
	objectStore = new Ext.data.GroupingStore({
            reader: objectReader,
			proxy: new Ext.data.HttpProxy({url: 'getAllContinent.shtml',method:'POST'}),
			remoteSort: true,
	...
	objectGrid = new Ext.grid.GridPanel({
		...
		columns: [
		     rn, //行号列 
		     cbsm, 
            {id:'continentId',header: '标识号', hidden:true, hideable:false, sortable: true, 
...
function updateItem(idName,start){
	...
	if (!selectRecord) {
		Ext.MessageBox.alert('修改操作', '请选择要修改的大洲！');
	}
	else {
		if (cbsm.getCount() == 1) {			
			updateTab(selectRecord.get('continentId'),selectRecord.get('name'),'getSingleContinent.shtml?continentId='+selectRecord.get('continentId'));
		}
		else {
...
function viewItem(start){
	selectRecord = objectGrid.getSelectionModel().getSelected();
	if (!selectRecord) {
		Ext.MessageBox.alert('查看操作', '请选择要查看的大洲！');
	}
	else {
		if (cbsm.getCount() == 1) {
			window.open('../../destination/continent/' + selectRecord.get('areaId')+'.html');
		}
		else {
			Ext.MessageBox.alert('提示', '请选择一条大洲信息！');
		}
