...
function initView(){
	...
	centerbody = new Ext.TabPanel({
		...
		items: [{
			...
			frame: true,
			autoLoad: {
				url: 'tabFrame.jsp?url=' + 'newsType.jsp?params=',
	...
	typeTree.on('beforeload', function(node){
		typeTree.loader.dataUrl = 'getTypeByNode.shtml';
	});
...
function clickNode(item){
	updateTab(item.id, item.text, 'newsType.jsp?params=' + item.id );
}
