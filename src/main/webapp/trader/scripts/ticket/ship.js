...
function initGrid(){
	...
	var objectReader = new Ext.data.JsonReader({
		...
		idProperty:'shipId',
        fields: [
            {name: 'shipId', type: 'string',mapping:'shipId'},
                         {name:'shipId'},
                         {name:'number'},
                         {name:'name'},
                         {name:'begin'},
...
function initUpdate(result,fpanel,idName){
	...
	if (strList.success === true) {
		...
		fpanel.findById(idName[4]).setValue(list.company);
		fpanel.findById(idName[5]).setValue(list.beginDateStr);
		fpanel.findById(idName[6]).setValue(list.begintime);
		fpanel.findById(idName[7]).setValue(list.price);
		fpanel.findById(idName[8]).setValue(list.total);
	}
}
...
function save(fpanel,start){
	...
	if (addWin) {
		urlPost = 'addShip.shtml';
	}
	else 
		if (updWin) {
			urlPost = 'updateShip.shtml';
			paramsPost = {
				shipId: selectRecord.get('shipId')
			};
		}
		else {
...
function updateItem(idName,start){
	...
	if (!selectRecord) {
		Ext.MessageBox.alert('修改操作', '请选择要修改的轮船信息！');
	}
	else {
		if (cbsm.getCount() == 1) {
			updWin = Ext.getCmp('updW');
			comCreate(idName,selectRecord.get('shipId'),start);
			...
			Ext.MessageBox.alert('提示', '请选择一条轮船信息！');
		}
	}
}
...
function delItem(start){
	selectRecord = objectGrid.getSelectionModel().getSelected();
	if (!selectRecord) {
		Ext.MessageBox.alert('轮船信息删除操作', '未选择，请选择要删除的轮船信息！');
	}
	else {
		Ext.MessageBox.confirm('确认删除', '删除轮船信息将同时删除与该轮船信息相关的团队，你确认删除选择的数据吗？', function(btn){
			if (btn == "yes") {
				var m = objectGrid.getSelections();
				var jsonData = "";
				for (var i = 0; i < m.length; i++) {
					var ss = m[i].get('shipId');
					if (i === 0) {
...
function delAjax(start,jsonData){
	Ext.Ajax.request({
		...
		success: function(result, request){
			...
			{
				Ext.MessageBox.alert('轮船信息删除操作', '删除轮船信息成功！');
			}
			else
			{
				Ext.MessageBox.alert('轮船信息删除操作', returnJosn.msg);
				ds.load({
					params: {
						start: start,
						limit: pageSize
		...
		failure: function(result, request){
			Ext.MessageBox.alert('轮船信息删除操作', doJSON(result.responseText).msg);
			objectStore.load({
					params: {
						start: start,
						limit: pageSize
