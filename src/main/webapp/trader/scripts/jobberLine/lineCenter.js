...
function initView(){
	...
	centerbody = new Ext.TabPanel({
		...
		items: [{
			...
			frame: true,
			autoLoad: {
				url: 'tabFrameRegionLine.jsp?url=' + 'lineRegion.jsp?params=' + regionId + '@@@' + regionId + '@@@' + flag,
...
function clickNode(item){
	updateTab(item.id, item.text, 'lineRegion.jsp?params=' + item.id + '@@@' + regionId + '@@@' + flag);
}
...
function addTab(id, tabTitle, targetUrl){
	centerbody.add({
		...
		tabTip: tabTitle,
		autoLoad: {
			url: "tabFrameRegionLine.jsp?url=" + targetUrl,
			callback: this.initSearch,
			scope: this,
			text: "Loading..."
		},
