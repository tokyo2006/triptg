...
function initView(){
	...
	centerbody = new Ext.TabPanel({
		...
		items: [{
			...
			frame: true,
			autoLoad: {
				url: 'tabFrame.jsp?url=' + 'pageContent.jsp?params=',
	...
	pageTree.on('beforeload', function(node){
		pageTree.loader.dataUrl = 'getPageSortList.shtml';
	});
...
function clickNode(item){
	updateTab(item.id, item.text, 'pageContent.jsp?params=' + item.id);
}
